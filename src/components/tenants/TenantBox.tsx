...
export function TenantBox() {
	...
	const selectedTenants: any[] =
		selectedTenantNames.length > 0 && tenants
			? tenants.filter((t: any) =>
					selectedTenantNames.includes(t.metadata?.name || ""),
				)
			: [];
			...
			? Array.from(
					new Set(
						selectedTenantNames.flatMap((name) => {
							...
							return Array.isArray(t?.status?.namespaces)
								? t.status.namespaces
								: [];
						}),
					),
	...
	useEffect(() => {
		localStorage.setItem(
			"selectedTenantNames",
			JSON.stringify(selectedTenantNames),
		);
	...
	const handleToggle = (tenant: TenantItem) => {
		...
		if (!wasSelected) {
			const banner =
				tenant.infoBanner ||
				(tenant as any).annotations?.["info.projectcapsule.dev/banner"];
			if (banner) {
	...
	const renderTenantItem = (tenant: TenantItem, isSelected: boolean) => {
		...
		const nsCount =
			typeof ns === "number" ? ns : (ns?.length ?? tenant.status?.size ?? 0);
	...
	const content = (
		<List sx={{ py: 0.5, minWidth: 220 }} dense>
			...
			>
				...
				<ListItemText
					primary="All Tenants"
					secondary="Show resources across all namespaces"
				/>
			</ListItem>
			<Divider sx={{ my: 0.5 }} />
			{tenants.length === 0 && (
