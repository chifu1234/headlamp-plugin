...
export function TenantDetail(props: TenantProps) {
	...
	const tenant = useMemo(
		() => tenants?.find((t: any) => t.getName() === name),
		[tenants, name],
	);
	...
	const nsVal =
		tenant?.status?.namespaces ?? tenant?.jsonData?.status?.namespaces;
	const numNamespaces =
		typeof nsVal === "number" ? nsVal : (nsVal?.length ?? 0);
	const state =
		tenant?.status?.state || tenant?.jsonData?.status?.state || "Active";
	...
	useEffect(() => {
		...
		const banner =
			tenant.infoBanner ||
			tenant.jsonData?.metadata?.annotations?.[
		...
		<>
			{tenantsError && (
				<Typography color="error" sx={{ mb: 2 }}>
					...
					{tenantsError.message || String(tenantsError)}. Is the Capsule CRD
			...
			<Resource.DetailsGrid
				...
				extraInfo={(item) => {
					...
					const description =
						...
						item.jsonData?.metadata?.annotations?.[
			...
			{showNotes && (
				...
				>
					<Typography variant="caption" display="block">
						Cordoning prevents tenant owners from modifying resources in the
						tenant's namespaces. Use the Cordon action in the resource header
						above.
					</Typography>
				</Box>
			)}
			...
			<SectionBox title="Conditions">
				...
				tenant.status.conditions.length === 0 ? (
...
function TenantNamespacesOverview({ tenant }: { tenant?: any }) {
	...
	const tenantNamespaces = useMemo(() => {
		...
		const spaces =
			tenant.status?.spaces || tenant.jsonData?.status?.spaces || {};
					...
					getValue: (item: any) => {
						...
						if (spaces && typeof spaces === "object") {
							if (spaces[nsName]) {
								...
								const arr = Array.isArray(spaces)
									? spaces
									: Object.values(spaces);
								spaceInfo = arr.find(
									(s: any) => s && (s.name === nsName || s === nsName),
								);
							}
						}
