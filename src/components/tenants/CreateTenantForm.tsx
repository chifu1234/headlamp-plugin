...
export default function CreateTenantForm(props: CreateTenantFormProps) {
	...
	const owners =
		currentOwners.length > 0 ? currentOwners : [{ kind: "User", name: "" }];
	...
	const updateAnnotation = (key: string, value: string) => {
		...
		const updated = {
			...
			metadata: {
				...(resource.metadata || {}),
				annotations:
					Object.keys(newAnnotations).length > 0 ? newAnnotations : undefined,
			},
		};
		onChange(updated);
	};
		...
		<Box>
			...
			<Box sx={{ mt: 1.5 }}>
				...
				<Typography
					...
					sx={{ mb: 1 }}
				>
					Users or Groups who will own this Tenant and can create/manage its
					Namespaces (see Capsule docs on Permissions).
				</Typography>
					...
					>
						<FormControl size="small" sx={{ minWidth: 110 }}>
							...
							<Select
								...
								label="Kind"
								onChange={(e) =>
									changeOwner(index, "kind", e.target.value as string)
								}
							>
			...
			<Box sx={{ mt: 2.5 }}>
				...
				<Typography
					...
					sx={{ mb: 1 }}
				>
					These are stored as annotations and improve the experience in the
					Capsule plugin (chooser, lists, detail, overview). See{" "}
					<a
				...
				<TextField
					...
					onChange={(e) =>
						updateAnnotation("info.projectcapsule.dev/icon", e.target.value)
					}
				...
				<TextField
					...
					onChange={(e) =>
						updateAnnotation("info.projectcapsule.dev/banner", e.target.value)
					}
			...
			<Accordion sx={{ mt: 2 }} defaultExpanded={false}>
				...
				<AccordionDetails>
					<Typography variant="body2" color="text.secondary">
						For advanced configuration (pod defaults, forbidden
						labels/annotations, resource pools, network policies, etc.) use the
						full YAML editor or edit the Tenant after creation. See{" "}
						<a
							href="https://projectcapsule.dev/docs/tenants/"
							target="_blank"
							rel="noreferrer"
						>
