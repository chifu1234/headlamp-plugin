...
export function CapsuleOverview() {
	...
	const allManagedApplied = useMemo(() => {
		const fromGlobal = (gTRs || []).flatMap((r: any) =>
			getAppliedObjectsForTable(r),
		);
		const fromTenant = (tTRs || []).flatMap((r: any) =>
			getAppliedObjectsForTable(r),
		);
	...
	const stats = useMemo(() => {
		...
		tenants?.forEach((t: any) => {
			...
			const isCordoned =
				!!t.jsonData?.spec?.cordoned || state.toLowerCase() === "cordoned";
			if (isCordoned) {
				cordoned++;
			} else {
				active++;
		...
		const gtrReadyCount = (gTRs || []).filter((r: any) =>
			isResourceReady(r),
		).length;
		const trReadyCount = (tTRs || []).filter((r: any) =>
			isResourceReady(r),
		).length;
		...
		(managedObjects || []).forEach((obj: any) => {
			...
			if (
				...
				statusInfo.label.toLowerCase() === "true"
			) {
				moReady++;
	...
	return (
		<>
			...
			<Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
				Overview of your Capsule multi-tenancy installation. Use the tenant
				selector in the top bar to scope the UI.
			</Typography>
			...
			<SectionBox title="Tenants">
				{tenantsError && (
					<Typography color="error" sx={{ mb: 1 }}>
						...
						{tenantsError.message || String(tenantsError)}
					</Typography>
				)}
				{nsError && (
					<Typography color="error" sx={{ mb: 1 }}>
