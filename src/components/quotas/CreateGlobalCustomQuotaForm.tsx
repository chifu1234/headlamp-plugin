...
export default function CreateGlobalCustomQuotaForm(
	props: CreateGlobalCustomQuotaFormProps,
) {
			...
			fields: [
				{
					...
					required: true,
					helperText:
						'Total allowed across selected namespaces. e.g. "500Gi" or "10"',
				},
			],
		},
	];
		...
		<Box>
			...
			<Box sx={{ mt: 1.5 }}>
				...
				<Typography
					...
					sx={{ mb: 1 }}
				>
					GlobalCustomQuota applies across many namespaces (typically via tenant
					label). Add namespaceSelectors.
				</Typography>
				...
				<TextField
					...
					value={
						nsSelectors[0]?.matchLabels?.["capsule.clastix.io/tenant"] || ""
					}
					onChange={(e) => {
						const val = e.target.value.trim();
						const sel = val
							? { matchLabels: { "capsule.clastix.io/tenant": val } }
							: {};
				...
				<Typography
					...
					sx={{ mb: 1 }}
				>
					Define GVK + operation. Use count for number of objects, add/sub +
					path for quantities (storage, cpu...). Full selectors documented at
					projectcapsule.dev.
				</Typography>
			...
			>
				ScopeSelectors, emitMetricPerClaimUsage and advanced selectors available
				after creation / via full YAML.
			</Typography>
		</Box>
	);
}
