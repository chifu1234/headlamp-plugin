	...
	const tenantResourceItem = useMemo(
		() =>
			list?.find(
				(q: any) =>
					q.getName() === name &&
					(!namespace || q.getNamespace() === namespace),
			),
		[list, name, namespace],
	);
		...
		<>
			{listError && (
				<Typography color="error" sx={{ mb: 2 }}>
					Error loading TenantResource: {listError.message || String(listError)}
					. Is the Capsule CRD present and do you have list permission?
				</Typography>
			)}
...
function TenantResourceConditions({
	...
	const item =
		...
		list?.find(
			(q: any) =>
				q.getName() === name && (!namespace || q.getNamespace() === namespace),
		);
	const conditions =
