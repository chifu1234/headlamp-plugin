...
export function GlobalTenantResourceDetail(
	props: GlobalTenantResourceDetailProps,
) {
		...
		<>
			{listError && (
				<Typography color="error" sx={{ mb: 2 }}>
					Error loading GlobalTenantResource:{" "}
					{listError.message || String(listError)}. Is the Capsule CRD present
					and do you have list permission?
				</Typography>
			)}
				...
				extraInfo={(item) => {
					...
					return [
						...
						},
						{
							...
							value: (
								<Typography>
									{item.spec?.resyncPeriod ||
										item.jsonData?.spec?.resyncPeriod ||
										"—"}
								</Typography>
							),
						},
			...
			<GlobalTenantResourceConditions name={name} item={tenantResourceItem} />
			<GlobalTenantResourceDefinedResources
				name={name}
...
function GlobalTenantResourceConditions({
	name,
	item: providedItem,
}: {
	name: string;
	...
	const conditions =
		item?.status?.conditions || item?.jsonData?.status?.conditions || [];
