...
type QuotaClaim = NonNullable<
	NonNullable<CustomQuota["status"]>["claims"]
>[number];
						...
						columns={[
							...
							},
							{
								...
								getter: (c: QuotaClaim) => {
									...
									const p =
										limitVal > 0
											? Math.min(100, Math.max(0, (u / limitVal) * 100))
											: 0;
									return (
										<Chip
											label={`${p.toFixed(0)}%`}
											color={usageChipColor(p)}
											size="small"
										/>
									);
								},
