...
function inventoryNameLink(item: KubeObject) {
	...
	const apiVersion: string =
		json?.apiVersion || (item as any)?.apiVersion || "v1";
	const slashIndex = apiVersion.lastIndexOf("/");
	const groupName =
		slashIndex > 0 ? apiVersion.substring(0, slashIndex) : apiVersion;
		...
		(applied || []).forEach((entry: any) => {
			...
			const doGet = (rc: any) => {
				rc.apiGet(
					(data: KubeObject) => {
						setResources((prev) => {
							if (prev.find((r) => r.metadata?.uid === data?.metadata?.uid))
								return prev;
							return [...prev, data];
						});
					},
...
export function GetResourcesFromApplied({
	applied,
	extraColumns,
}: GetResourcesFromAppliedProps) {
...
function renderYAMLWithSSAHighlight(
	obj: any,
	fieldsV1: any,
	level = 0,
): React.ReactNode {
			...
			elements.push(
				<div key={index} style={lineStyle}>
					...
					{typeof item === "object" && item !== null
						? renderYAMLWithSSAHighlight(
								item,
								getSubFieldsV1(fieldsV1, ""),
								level + 1,
							)
						: JSON.stringify(item)}
					{managed && (
						<span style={{ color: "#4ec9b0", marginLeft: 4 }}>★</span>
					)}
				</div>,
			);
		});
		...
		Object.entries(obj).forEach(([key, value]) => {
			...
			elements.push(
				<div key={key} style={lineStyle}>
					{indent}
					<span
						style={{ color: keyColor, fontWeight: managed ? "bold" : "normal" }}
					>
						{key}:
					</span>
					...
					{managed && (
						<span
							style={{ color: "#4ec9b0", marginLeft: 6, fontSize: "0.8em" }}
						>
							★ SSA
						</span>
					)}
...
export function ManagedResources({
	...
	function inspectFor(item: KubeObject) {
		const apiVersion =
			(item as any)?.apiVersion || item?.jsonData?.apiVersion || "v1";
		setSelected({
			apiVersion,
			kind: item?.kind,
		...
		return (
			<SectionBox title={title}>
				<Typography color="text.secondary">
					No managed resources reported yet.
				</Typography>
			</SectionBox>
		);
	}
				...
				>
					{fetchError && (
						<Typography color="error">
							Error fetching object: {fetchError}
						</Typography>
					)}
