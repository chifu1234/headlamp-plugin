...
export function TenantResourceReconcileAction(props: any) {
	...
	if (
		kind !== "TenantResource" ||
		!apiVersion?.startsWith("capsule.clastix.io/")
	) {
		return null;
	}
...
export function TenantCordonAction(props: any) {
	const [optimisticCordoned, setOptimisticCordoned] = useState<boolean | null>(
		null,
	);
	const { enqueueSnackbar } = useSnackbar();
	...
	const isCordoned =
		optimisticCordoned !== null
			? optimisticCordoned
			: !!resource.jsonData?.spec?.cordoned;
	...
	const handleCordon = async () => {
		...
		try {
			...
			await ApiProxy.patch(patchUrl, patchBody);
			enqueueSnackbar(
				`Tenant ${target ? "cordoned" : "uncordoned"} successfully`,
				{
					variant: "success",
				},
			);
	...
	const handleCordon = async () => {
		...
		if (resource.jsonData) {
			...
			if (!resource.jsonData.metadata.labels)
				resource.jsonData.metadata.labels = {};
			if (target) {
				resource.jsonData.metadata.labels["projectcapsule.dev/cordoned"] =
					"true";
			} else {
		...
		try {
			...
			await ApiProxy.patch(patchUrl, patchBody);
			enqueueSnackbar(
				`Namespace ${target ? "cordoned" : "uncordoned"} successfully`,
				{
					variant: "success",
				},
			);
			...
			if (resource.jsonData?.metadata?.labels) {
				if (current) {
					...
					delete resource.jsonData.metadata.labels[
