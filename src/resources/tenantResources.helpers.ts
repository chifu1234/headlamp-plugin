...
export function isResourceReady(item: any): boolean {
	...
	const conditions: any[] =
		j?.status?.conditions || item?.status?.conditions || [];
...
export function getDefinedReplicationEntries(rules: any[]): any[] {
	...
	(rules || []).forEach((rule: any, ruleIdx: number) => {
		...
		const hasConcrete =
			...
			(rule.namespacedItems?.length || 0) +
				(rule.rawItems?.length || 0) +
				genCount >
				0;
		if (!hasConcrete) {
			entries.push({
				kind: "Rule",
				name: rule.namespaceSelector
...
export function getManagedObjectReadyStatus(
	...
	if (status && Array.isArray(status.conditions)) {
		const hasReadyCond = status.conditions.some(
			(c: any) => c && c.type === "Ready",
		);
		if (hasReadyCond) {
	...
	const desc = (appliedDescriptors || []).find((a: any) => {
		...
		const oApi =
			(liveObj as any)?.apiVersion || liveObj?.jsonData?.apiVersion || "";
		return (
	...
	if (pStatus !== null && pStatus !== undefined) {
		...
		if (
			["true", "success", "applied", "ready", "succeeded"].some((s) =>
				lower.includes(s),
			)
		) {
			color = "success";
		} else if (
			["false", "error", "failed", "fail"].some((s) => lower.includes(s))
		) {
			color = "error";
		}
		return { label, color };
