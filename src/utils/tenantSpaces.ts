...
export function getTenantSpaces(tenant: any): SpaceInfo[] {
  ...
  if (Array.isArray(spaces)) {
    return spaces.map((s: any) => (typeof s === 'string' ? { name: s } : (s as SpaceInfo)));
  }
  return Object.entries(spaces).map(([name, info]) =>
    info && typeof info === 'object' ? { name, ...(info as SpaceInfo) } : { name }
  );
}
...
export function findSpaceInfo(tenant: any, nsName: string): SpaceInfo | undefined {
  const spaces = rawSpaces(tenant);
  ...
  return getTenantSpaces(tenant).find(s => s.name === nsName) as SpaceInfo | undefined;
}

/** Finds the Ready condition (or the first condition) of a space entry. */
export function findReadyCondition(spaceInfo: SpaceInfo | undefined): SpaceCondition | undefined {
  const conditions = spaceInfo?.conditions || [];
  if (conditions.length === 0) return undefined;
  return (
    conditions.find(
      c =>
        c.type === 'Ready' || (typeof c.type === 'string' && c.type.toLowerCase().includes('ready'))
    ) || conditions[0]
  );
}
